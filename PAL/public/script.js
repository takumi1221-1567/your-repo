document.addEventListener('DOMContentLoaded', () => {
    ...
    const speak = (text, finalImage = NORMAL_IMAGE) => { if (lipSyncInterval) clearInterval(lipSyncInterval); window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); const voices = window.speechSynthesis.getVoices(); utterance.voice = voices.find(voice => voice.lang === 'ja-JP') || voices[0]; utterance.rate = 1.0; utterance.pitch = 1.1; utterance.onstart = () => { let isMouthOpen = false; lipSyncInterval = setInterval(() => { changeCharacterImage(isMouthOpen ? finalImage : TALKING_IMAGE); isMouthOpen = !isMouthOpen; }, 150); }; utterance.onend = () => { clearInterval(lipSyncInterval); changeCharacterImage(finalImage); }; window.speechSynthesis.speak(utterance); };
    ...
    const askDify = async (question) => {
        ...
        try {
            ...
            if(response.headers.get('content-type')?.includes('application/json')){
                ...
                speak(data.answer, NORMAL_IMAGE);
                return;
            }
